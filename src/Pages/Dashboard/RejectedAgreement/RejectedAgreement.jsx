...
const RejectedAgreement = () => {
  ...
  const { data: rejectedUsers = [], refetch } = useQuery({
    queryKey: ["rejectedUsers"],
    queryFn: async () => {
      const res = await axiosSecure.get("/rejectedUsers")
      return res.data;
    }
  })
    ...
    <div className="p-7 bg-red-300 min-h-screen">
      ...
      <div className="mt-10">
        <table className="table">
          ...
          <tbody>
            {
              rejectedUsers.map((data, i) => <tr key={data._id}>
...
export default RejectedAgreement;